...
int DFT_Test<precision, domain>::test_out_of_place_real_real_USM() {
    ...
    try {
        ...
        auto ua_output = usm_allocator<PrecisionType, usm::alloc::shared, 64>(cxt, *dev);
        ...
        event done = dft::compute_forward<std::remove_reference_t<decltype(descriptor)>,
        ...
        commit_descriptor(descriptor_back, sycl_queue);
                                     ...
                                     PrecisionType, PrecisionType>(
            descriptor_back, out_re.data(), out_im.data(), out_back_re.data(), out_back_im.data());
        done.wait();
    }
...
int DFT_Test<precision, domain>::test_out_of_place_real_real_buffer() {
    ...
    try {
        ...
        dft::compute_backward<std::remove_reference_t<decltype(descriptor_back)>, PrecisionType,
                              PrecisionType>(descriptor_back, out_dev_re, out_dev_im,
                                             out_back_dev_re, out_back_dev_im);
    }
    catch (oneapi::mkl::unimplemented &e) {
        std::cout << "Skipping test because: \"" << e.what() << "\"" << std::endl;
        return test_skipped;
