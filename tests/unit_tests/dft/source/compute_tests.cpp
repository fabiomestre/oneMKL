...
TEST_P(ComputeTests, RealSinglePrecisionInPlaceBuffer) {
    auto test = DFT_Test<dft::precision::SINGLE, dft::domain::REAL>{ std::get<0>(GetParam()),
                                                                     std::get<1>(GetParam()) };
    EXPECT_TRUEORSKIP(test.test_in_place_buffer());
}

TEST_P(ComputeTests, RealDoublePrecisionInPlaceBuffer) {
    auto test = DFT_Test<dft::precision::DOUBLE, dft::domain::REAL>{ std::get<0>(GetParam()),
                                                                     std::get<1>(GetParam()) };
    EXPECT_TRUEORSKIP(test.test_in_place_buffer());
}

TEST_P(ComputeTests, ComplexSinglePrecisionInPlaceBuffer) {
    auto test = DFT_Test<dft::precision::SINGLE, dft::domain::COMPLEX>{ std::get<0>(GetParam()),
                                                                        std::get<1>(GetParam()) };
    EXPECT_TRUEORSKIP(test.test_in_place_buffer());
}

TEST_P(ComputeTests, ComplexDoublePrecisionInPlaceBuffer) {
    auto test = DFT_Test<dft::precision::DOUBLE, dft::domain::COMPLEX>{ std::get<0>(GetParam()),
                                                                        std::get<1>(GetParam()) };
    EXPECT_TRUEORSKIP(test.test_in_place_buffer());
}
